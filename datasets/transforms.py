...
class GroupNormalize(object):
    ...
    def __call__(self, Te):
        tensor, label = Te   #tensor.size:(24,224,224)
        mean = self.mean * (tensor.size()[0]//len(self.mean))  # list * n：复制几遍
        std = self.std * (tensor.size()[0]//len(self.std))   #每3个对应一张照片，完美
        mean = torch.Tensor(mean)
        std = torch.Tensor(std)

        if len(tensor.size()) == 3:
            # for 3-D tensor (T*C, H, W)
            tensor.sub_(mean[:, None, None]).div_(std[:, None, None])  #normalization
...
class GroupScale(object):
    ...
    def __call__(self, img):
        # print('type of img',type(img))    #tuple
        img_group, label = img
        # print('type of img_group',type(img_group))     #list
        return [self.worker(img) for img in img_group],label
