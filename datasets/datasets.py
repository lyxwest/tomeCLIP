...
class Video_dataset(data.Dataset):
    ...
    def _parse_list(self):
        ...
        tmp = [x.strip().split(' ') for x in open(self.list_file)]  #视频名称 总帧数 label
    ...
    def _sample_indices(self, video_list):
        if self.dense_sample:
            ...
            ticks = [i * len(video_list) // self.num_segments
                    for i in range(self.num_segments + 1)]    #长度为num_segments+1的index的list

            for i in range(self.num_segments):
                tick_len = ticks[i + 1] - ticks[i]   #每一个segment的长度（有几帧）
                tick = ticks[i]
    ...
    def _get_test_indices(self, video_list):
        if self.dense_sample:
            ...
            num_clips = self.test_clips
            #print('--------------',len(video_list), self.num_segments)
            tick = len(video_list) / float(self.num_segments)   #每个segment有多少帧
            start_list = np.linspace(0, tick - 1, num=num_clips, dtype=int)
            offsets = []
            for start_idx in start_list.tolist():
                offsets += [
                    int(start_idx + tick * x) % (len(video_list)+1) for x in range(self.num_segments)
                ]     #每个segment的第一帧的index加入到offsets
            return np.array(offsets) + self.index_bias   #把offset变成array，然后+1
    ...
    def __getitem__(self, index):
        ...
        if self.modality == 'video':
            ...
            record = self.video_list[index]   #record是第index个视频，包含三个元素：path，label和帧总数num_frames
            #print('record',record.path)      #smoke/Rich_smoking_experience_smoke_h_nm_np1_fr_med_1
            video_list = os.listdir(os.path.join(self.root_path, record.path))  #包含当前视频的所有切帧图片
            #print('video list', len(video_list))     #record.path下的图片
    ...
    def _load_image(self, directory, idx):
        if self.modality == 'RGB':
            try:
                #idx对应图片的地址，idx是前面设计的：每个segment的第一张
    ...
    def get(self, record, video_list, indices):
        ...
        for seg_ind in indices:
            ...
            # print(type(seg_imgs))    #list
            images.extend(seg_imgs)
            if p < len(video_list):
                p += 1
        #print('1:',len(images), type(images)) #8   list
        #print(images)     #8张image
        #print('1111',type(images[0]))
        process_data, record_label = self.transform((images,record.label))
        #print('2:',type(process_data))     #tentor
        #print('3:', process_data.size())    #torch.Size([24, 224, 224])
        #print(type(record_label),record_label)   #int 对应一个class id
