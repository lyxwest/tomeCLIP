...
def update_dict(dict, tome):
    ...
    if tome > 0:
        print("changing checkpoints clip-style to attention style")
    
    for k, v in dict.items():
        if tome > 0:
            for src, dst in src_to_dst_keys:
                if k.count(src)>0 and k.count("fusion")==0:
                    k = k.replace(src, dst)
...
def main(args):
    ...
    if config.tome > 0:
        ...
        patch_openclip(model, config.tome)
        print("using tome,ratio={}".format(config.tome))
    ...
    if os.path.isfile(args.weights):
        ...
        model_full.load_state_dict(update_dict(checkpoint['model_state_dict'], config.tome))
        del checkpoint
